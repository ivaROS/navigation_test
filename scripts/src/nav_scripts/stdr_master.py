...
from nav_scripts.stdr_testing_scenarios import TestingScenarios
...
import datetime
import tf2_ros
from nav_scripts.stdr_testing_scenarios import SectorScenario, CampusScenario, FourthFloorScenario, SparseScenario
import rosbag
...
class ResultRecorder:
    def __init__(self, taskid):
        ...
        bagpath = "~/simulation_data/quadruped_benchmark/quadruped_stdr/bagfile/" + str(datetime.datetime.now()) + "_" + str(taskid) + ".bag"
...
class MultiMasterCoordinator:
    def __init__(self, num_masters=4, record=False):
        ...
        self.fieldnames.extend(["taskid","world", "pid", "result","time","path_length","robot","controller_robot"])
    ...
    def processResults(self,my_queue):
        ...
        outputfile_name = "~/simulation_data/sgap_benchmark/stdr/v2/results_1_sg_fast_0.4_b_pose_noff_4_4_0.5_fix6_rfix_" + str(datetime.datetime.now())
    ...
    def addTasks(self):
        worlds = ["office_laser"] # "dense_laser", "campus_laser", "office_laser", "sector_laser"
        fovs = ['360'] # ['90', '120', '180', '240', '300', '360']
        # seeds = list(range(25))
        seeds = [5,15,15,20]
        seeds = [ele for ele in seeds for i in range(1)]
        controllers = ['bgap_mpc_casadi'] # 'teb', 'bgap_mpc_casadi'
        taskid = 0

        # Nonholonomic Potential Gap Experiments
        # for world in ["office_laser"]:
        controller_robot = 'nonholonomic' # holonomic_po
        for world in worlds:
            for robot in ['nonholonomic']: # 'holonomic', 'nonholonomic'
                    ...
                    for fov in fovs:
                        for seed in seeds:
                            task = {
                                    ...
                                    "taskid" : taskid,
                                    'controller_args': {
                                        'far_feasible' : str((world != 'office_laser' and world != 'campus_laser')),
                                        'v_des' : 0.4,
                                        'use_bezier' : str(True),
                                        'use_pose_controller' : str(True),
                                        'use_keyhole' : str(False),
                                        'use_ni' : str(False),
                                        'use_po' : str(False),
                                        'has_feedforward' : str(False)
                                        }
                                    }
                            taskid += 1
                            self.task_queue.put(task)
...
class STDRMaster(mp.Process):
                ...
                if True:
                    ...
                    if task["controller"] is None:
                        ...
                        try:
                            ...
                            self.roslaunch_controller(task["controller_robot"], task["controller"], controller_args)
    ...
    def roslaunch_controller(self, robot, controller_name, controller_args={}):
        ...
        rospack = rospkg.RosPack()
        path = rospack.get_path("bezier_gap_benchmark")
        ...
        self.controller_launch = roslaunch.parent.ROSLaunchParent(
            run_id=uuid, roslaunch_files=[path + "/launch/controller/stdr/" + controller_name + "_" + robot + "_controller.launch"],
    ...
    def roslaunch_stdr(self, task):
        ...
        with self.stdr_launch_mutex:
            self.stdr_launch = roslaunch.parent.ROSLaunchParent(
                run_id=uuid, roslaunch_files=[path + "/launch/environment/stdr/" + launch_file_name],
    ...
    def move_robot(self, location):
        print("Moving robot to" + str(location.position.x) + " " + str(location.position.y))
        # print location
